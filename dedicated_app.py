...
def render_check_view(provider: str, offers_df: pd.DataFrame,
    ...
    if provider == "timeweb_cloud":
        st.caption("Ссылка открывает Санкт-Петербург (?location=ru). В таблице — "
                   "ПОМЕСЯЧНАЯ цена, как её сверяет клиент; цена вкладки "
                   "«12 месяцев, скидка 10 %» — в колонке «Условия цены».")
    if mark.get("stale"):
