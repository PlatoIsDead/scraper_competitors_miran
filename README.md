...
### Сверка с сайтами
```bash
python -m reconcile --fresh                                  # прогон пайплайна + сверка каждой пары
python -m reconcile --report data/reports/matches_20260915.csv   # сверить готовый отчёт
```
Отчёт — `data/reports/reconcile_<дата>.md` и `.csv`: строка на пару отчёта (✓/✗, причина, ссылка на карточку)
и список карточек конкурента с тем же CPU, не попавших в пары. ✗ = карточки нет или она скрыта, либо цена,
CPU, RAM или диски на карточке отличаются от строки отчёта (код выхода 1; 2 — витрина недоступна).
